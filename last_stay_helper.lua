		                ...
		                if k then
		                    ...
		                    if updateversion ~= thisScript().version then
		                        lua_thread.create(function(b)
		                            local d = require('moonloader').download_status
		                            local m = -1
		                            sampAddChatMessage(b..'Îáíàðóæåíî îáíîâëåíèå. Ñêà÷èâàåì îáíîâëåíèå '..updateversion, m)
		                            wait(250)
		                            downloadUrlToFile(updatelink, thisScript().path, function(n, o, p, q)
		                                if o == d.STATUS_ENDDOWNLOADDATA then
		                                    sampAddChatMessage(b..'Îáíîâëåíèå ñêðèïòà óñïåøíî çàâåðøåíî!', m)
		                                    goupdatestatus = true
		                                ...
		                                if o == d.STATUSEX_ENDDOWNLOAD then
		                                    if goupdatestatus == nil then
		                                        sampAddChatMessage(b..'Íå óäàëîñü îáíîâèòü ñêðèïòà íà âåðñèþ ñêðèïòà '..updateversion..'. Çàïóñêàåì ñòàðóþ âåðñèþ ñêðèïòà!', m)
		                                        update = false
		                                    end
		                                end
		                            end)
...
local server_items = { 
	{ {2663, "Ñóõîé ïàåê", "dry_food"}, {19882, "Æàðåíîå ìÿñî", "fry_meat"}, {19582, "Ñûðîå ìÿñî", "raw_meat"}, {2814, "Ïèööà", "pizza"}, {2768, "Áóðãåð", "burger"}, {19576, "ßáëîêî", "apple"}, {1666, "Áàíêà áîáîâ", "can_of_beans"}, {19566, "Ðûáíûå ïîëóôàáðèêàòû", "semi_fished_products"}, {1509, "Áóòûëêà", "bottle"}, {1546, "Sprunk", "sprunk"}, {19564, "ßáëî÷íûé ñîê", "apple_juice"}, {19563, "Àïåëüñèíîâûé ñîê", "orange_juice"}, {2856, "Ìîëîêî", "milk"} }, 
	{ {18634, "Ëîì", "crowbar"}, {1650, "Êàíèñòðà", "fuelcan"}, {3013, "Çàï÷àñòè", "spare_parts"}, {19918, "Àêêóìóëÿòîð", "battery"}, {1074, "Êîëåñî", "wheel"}, {341, "Áåíçîïèëà", "chainsaw"} },
	{ {2922, "Êîäîâûé çàìîê", "code_lock"}, {1672, "Êðàñíûé ôàåð", "red_fire"}, {1279, "Õðàíèëèùå", "storage"} } 
}

--local reasons = {"Êóëàê", "Êàñòåò", "Êëþøêà äëÿ ãîëüôà", "Ïîëèöåéñêàÿ äóáèíêà", "Íîæ", "Áåéñáîëüíàÿ áèòà", "Ëîïàòà", "Êèé", "Êèé", "Êàòàíà", "Áåíçîïèëà", "Áîëüøîé äèëäî", "Ìàëûé äèëäî", "Áîëüøîé âèáðàòîð", "Ìàëûé âèáðàòîð", "Öâåòû", "Òðîñòü", "Ãðàíàòà", "Ñëåçîòî÷èâûé ãàç", "Êîêòåéëü ìîëîòîâà", "Colt .45", "Colt .45 (ãëóøèòåëü)", "Desert Eagle", "Shotgun", "Sawed Off", "Combat Shotgun", "Uzi", "MP5", "AK-47", "M4", "Tec-9", "Country Rifle", "Sniper Rifle", "RPG", "RPG (HeatSeeker)", "Flamethrower", "Minigun", "Satchel", "Detonator", "SprayCan", "Vehicle"}
...
function main()
	...
	if autoupdate_loaded and enable_autoupdate and Update then
        ...
        sampAddChatMessage("{FFFF55}[LastStayHelper]{FFFFFF} - Íàéäåíà íîâàÿ âåðñèÿ ñêðèïòà!", -1)
    end

	if not sampIsLocalPlayerSpawned() then
		sampAddChatMessage("{FFFF55}[LastStayHelper]{FFFFFF} - Ñêðèïò çàãðóæåí!", -1)
	else
		sampAddChatMessage("{FFFF55}[LastStayHelper]{FFFFFF} - Ñêðèïò ïåðåçàãðóæåí!", -1)
	end
	...
	while true do wait(0)
		...
		if isCharInAnyCar(playerPed) then
			...
			renderFontDrawText(health_vehicle_font, "Çäîðîâüå: "..math.floor(math.abs(health_procent)).." %", 1000, 555, -1)
		end
		...
		if isKeyJustPressed(key.VK_L) and not sampIsDialogActive() and not sampIsChatInputActive() then
			...
			sampAddChatMessage(toggle_auto_crowbar_lambing == true and "[Óâåäîìëåíèå] {FFFFFF}Âû âêëþ÷èëè àâòî-äîáû÷ó ëîìîì!" or "[Óâåäîìëåíèå] {FFFFFF}Âû âûêëþ÷èëè àâòî-äîáû÷ó ëîìîì!", 0xFFFFAA2A)
		...
		if isKeyJustPressed(key.VK_K) and not sampIsDialogActive() and not sampIsChatInputActive() then
			...
			sampAddChatMessage(toggle_auto_fill_fuel_can == true and "[Óâåäîìëåíèå] {FFFFFF}Âû âêëþ÷èëè àâòî-çàïðàâêó êàíèñòð!" or "[Óâåäîìëåíèå] {FFFFFF}Âû âûêëþ÷èëè àâòî-çàïðàâêó êàíèñòð!", 0xFFFFAA2A)
...
function imgui.OnDrawFrame()
	...
	if last_stay_helper_window.v then
		...
		if imgui.CollapsingHeader(u8"Êàòåãîðèè ïðåäìåòîâ") then
			if imgui.Checkbox(u8"Ïðîðèñîâêà ïðåäìåòîâ", wallhack_items_check_box) then
			...
			if imgui.CollapsingHeader(u8"Ïðîâèçèÿ") then
			...
			if imgui.CollapsingHeader(u8"Ìåõàíèêà") then
			...
			if imgui.CollapsingHeader(u8"Ðåäêèå ïðåäìåòû") then
		...
		if imgui.CollapsingHeader(u8"Íàñòðîéêè") then
			...
			imgui.Text(u8"Äèñòàíöèÿ ïðîðèñîâêè ïðåäìåòîâ")
			...
			imgui.Text(u8"Ðàçìåð øðèôòà ïðåäìåòà")
			...
			if imgui.Button(u8"Ñáðîñèòü öâåò", reset_draw_font_color) then
...
function ProcessAllItemsCheckbox(index)
	if imgui.Checkbox(u8"Âñå", toggle_all_items[index]) then
...
function hook.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if dialogid == DIALOG_INVENTORY_ID then
		if toggle_auto_crowbar_lambing == true then
			...
			for line in text:gmatch('[^\r\n]+') do
				...
				if line_index ~= 0 then
					for _, quality in pairs(item_qualities) do					
						if line:find(quality.."|{FFFFFF} Ëîì") then
			...
			sampAddChatMessage("[Óâåäîìëåíèå] {FFFFFF}Ó âàñ íåò ëîìà â èíâåíòàðå èëè îí ñëîìàí!", 0xFFFFAA2A)
			toggle_auto_crowbar_lambing = false
		...
		elseif toggle_auto_fill_fuel_can == true then
			...
			for line in text:gmatch('[^\r\n]+') do
				...
				if line_index ~= 0 then
					for _, quality in pairs(item_qualities) do					
						if line:find(quality.."|{FFFFFF} Ïóñòàÿ êàíèñòðà") then
							sampSendDialogResponse(dialogid, 1, line_index - 1, "")
							sampCloseCurrentDialogWithButton(1)
			...
			sampAddChatMessage("[Óâåäîìëåíèå] {FFFFFF}Ó âàñ íåò êàíèñòðû â èíâåíòàðå èëè îíà ñëîìàíà!", 0xFFFFAA2A)
			toggle_auto_fill_fuel_can = false
		end
		if use_pain_killer == true then
			...
			for line in text:gmatch('[^\r\n]+') do
				...
				if line_index ~= 0 then
					for _, quality in pairs(item_qualities) do					
						if line:find(quality.."|{FFFFFF} Îáåçáîëèâàþùåå") then
							sampSendDialogResponse(dialogid, 1, line_index - 1, "")
							sampCloseCurrentDialogWithButton(1)
							return
						end
			...
			sampAddChatMessage("[Óâåäîìëåíèå] {FFFFFF}Ó âàñ íåò îáåçáîëèâàþùèõ â èíâåíòàðå èëè îíà ñëîìàíà!", 0xFFFFAA2A)
...
function hook.onServerMessage(color, text)
	if toggle_auto_crowbar_lambing == true then
		if text:find("{FFFFFF} Ïåðåä âàìè íåò íè îäíîãî ðæàâîãî àâòî") then
			toggle_auto_crowbar_lambing = false
		end

		if text:find("Âû óñïåøíî äîáûëè íåîáðàáîòàííîå æåëåçî") or text:find("Âû óñïåøíî äîáûëè æåëåçíóþ ïëàñòèíó") then
			simulateKeyPress(key.VK_Y)
		end
	end

	if toggle_auto_fill_fuel_can == true then
		if text:find("Âû íå íà çàïðàâêå è îêîëî âàñ íåò Ò/Ñ") or text:find("Íà ýòîé çàïðàâêå íåò òîïëèâà!") then
			toggle_auto_fill_fuel_can = false
		end

		if text:find("Âû íàïîëíèëè Êàíèñòðó") then
			simulateKeyPress(key.VK_Y)
		end
	end

	if use_pain_killer == true then
		if text:find("Âû íå íóæäàåòåñü â îáåçîáëèâàþùåì!") then
			use_pain_killer = false
		end
	end
end
